...
set.seed(1234) # guarantee all get the same ramdon number
x1 <- rnorm(5)
...
cylsq = (mtcars$cyl)^2 # create a new vector
mtcars$cylsq = (mtcars$cyl)^2  #create a new column in mtcars
...
# Hint: use table()  default order:alphabet
...
gapminder07 = gapminder[gapminder$year == 2007,]  # !there is a comma
...
t = t.test(gapminder07$gdpPercap~gapminder07$lifeExp_highlow)
...
hist(gapminder07$lifeExp,labels = TRUE)
?hist
# Re-create the histogram with a title and axis labels
hist(gapminder07$lifeExp,labels = TRUE,main = 'HaHa', xlab = 'lifeExp')
...
plot(gapminder07$lifeExp~gapminder07$gdpPercap, main = 'Title2', xlab ='gdpPercap', ylab = 'lifeExp')
